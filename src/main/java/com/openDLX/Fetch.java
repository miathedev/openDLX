...
public class Fetch {
	...
	private void setPc(uint32 pc)
	{
		...
		log.debug("Set Pc to: " + program_counter.getValueAsHexString());
	}
	...
	public void increasePC()
	{
		...
		log.debug("Pc is now at: " + program_counter.getValueAsHexString());
	}
	...
	public FetchOutputData doCycle() throws MemoryException
	{
		...
		{
			log.debug("speculatively jumping from " + bpmfd.getPc().getValueAsHexString() + " to " + bpmfd.getBranchTgt().getValueAsHexString());
		...
		if((ArchCfg.isa_type == ISAType.MIPS) || (ArchCfg.use_load_stall_bubble == true))
		{
			...
			{
				log.debug("mispredicted branch at pc " + efd.getPc().getValueAsHexString() + " the branch was actually " + ((efd.getJump())?("taken to " + efd.getNewPc().getValueAsHexString()):("not taken next instr is " + new uint32(efd.getPc().getValue()+8).getValueAsHexString())));
...
//				}
		...
		{
			log.debug("likely branch was not taken, flushing branch delay slot");
		...
		if(instr != null)
		{
			log.debug("PC: " + getPc().getValueAsHexString() + " fetched instruction " + instr.getValueAsHexString());
		}
		else
		{
			// stalling
