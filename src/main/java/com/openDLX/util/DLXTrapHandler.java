...
import com.openDLX.datatypes.*;
 import com.openDLX.exception.DLXTrapException;
 import com.openDLX.exception.MemoryException;
 import com.openDLX.gui.dialog.Input;

@Log4j2(topic = "PrintHandler")
public class DLXTrapHandler {
	...
	{
		log.error("TRAP " + PipelineConstants.DLX_TRAP_OPEN + " not implemented. Parameter: " + parameter + " mem");
	...
	{
		log.error("TRAP " + PipelineConstants.DLX_TRAP_CLOSE + " not implemented. Parameter: " + parameter + " mem");
		mem.dumpMemory(new uint32(parameter), new uint32(parameter+32));
		throw new DLXTrapException("TRAP " + PipelineConstants.DLX_TRAP_OPEN + " not implemented. Parameter: " + parameter);
	}
...
//		uint32 unknown = new uint32(parameter);
		...
		if(user_input != null)
		{
			...
			log.info("Input: " + user_input + " @" + write_addr.getValueAsHexString());
	...
	{
		log.error("TRAP " + PipelineConstants.DLX_TRAP_WRITE + " not implemented. Parameter: " + parameter + " mem");
		mem.dumpMemory(new uint32(parameter), new uint32(parameter+32));
	...
	public void printf(int parameter) throws MemoryException, DLXTrapException 
	{
		...
		log.debug("TRAP " + PipelineConstants.DLX_TRAP_PRINTF + " catched. Printf format string is at: " + format_addr.getValueAsHexString() + " Parameter list begins at: " + parameter_list_pointer.getValueAsHexString());
		
//		log.debug("Format string dump:");
//		mem.dumpMemory(format_addr, new uint32(format_addr.getValue() + 20));
//		log.debug("Parameter list dump:");
//		mem.dumpMemory(parameter_list_pointer, new uint32(parameter_list_pointer.getValue() + 20));
		...
		{
//			log.debug("Read addr: " + format_addr.getHex() + " char: " + (char)mem.read_u8(format_addr).getValue() + "("+ mem.read_u8(format_addr).getHex() +")");
			
			read_char = (char)(mem.read_u8(format_addr).getValue());
			String read_s = String.valueOf(read_char);
			format_string += read_s;
			
//			log.debug("Added :" + read_s + " to: " + format_string);
		...
		while(format_string_index != -1)
		{
			...
			{
//				log.debug("Found format delimiter: " + format_string.substring(format_descr_pos, format_descr_pos+1) + " reading from: " + parameter_list_pointer.toString());
			...
			{
//				log.debug("End of format string. start: " + format_string_index + " end: " + format_string.length());
		...
		log.info("Printf out: " + print_string);
