...
{	
	...
	public PagedMemory()
	{
		...
		log.debug("Reserving " + page_count + " pages, using the upper " + page_addr_bits + " bits for page addressing");
	}
	...
	public void writeByte(int address, byte data) throws MemoryException
	{
		...
		//log.trace("Accessing page: " + page_number + " 0x" + Integer.toHexString(page_number));
	...
	public byte readByte(int address) throws MemoryException
	{
		...
		if(pages[page_number] == null)
		{
			log.warn("Reading from unallocated page!");
			pages[page_number] = new Page();
		}
