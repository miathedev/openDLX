...
public class WriteBack
{
	...
	public WritebackOutputData doCycle()
	{
		...
		{
			log.info("Caught BREAK instruction - finishing simulation.");
			caught_break = true;
		}
		...
		{
			log.info("Caught TRAP 0 - finishing simulation.");
			caught_break = true;
		}
		...
		log.info("PC: " + pc.getValueAsHexString());
		if (inst.getLoad())
		{
		...
		if (regWrite)
		{
			...
			{
				log.debug("writing: " + regWriteValue.getValueAsHexString() + " to register " + regWriteSelect.getValue() + "/" + ArchCfg.getRegisterDescription(regWriteSelect.getValue()));
				reg_set.write(regWriteSelect, regWriteValue);
			}
			else
			{
				log.info("suppressing writing of register 0/" + ArchCfg.getRegisterDescription(0) + " with value: " + regWriteValue.getValueAsHexString());
			}
		}
		...
		if (inst.getWriteLO() || inst.getWriteHI())
		{
			...
			if (inst.getWriteLO())
			{
				log.debug("writing: " + alu_outLO.getValueAsHexString() + " to register " + SpecialRegisters.LO);
				reg_set.write_SP(SpecialRegisters.LO, alu_outLO);
			}

			if (inst.getWriteHI())
			{
				log.debug("writing: " + alu_outHI.getValueAsHexString() + " to register " + SpecialRegisters.HI);
