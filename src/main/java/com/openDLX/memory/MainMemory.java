...
import java.io.IOException;
 ...
 import com.openDLX.util.PagedMemory;

import lombok.extern.log4j.Log4j2;

@Log4j2(topic = "MainMemory")
public class MainMemory implements MemoryInterface
{
    ...
    public uint32 read_u32(uint32 address, boolean log_output) throws MemoryException
    {
        ...
        {
            log.error("Read u32 from unaligned addr: " + address.getValueAsHexString());
        ...
        if (log_output)
        {
            //log.trace("Read u32 from addr: " + address.getHex());
            //log.trace("Read: " + Integer.toHexString(memory.readByte(address.getValue())) + " " + Integer.toHexString(memory.readByte(address.getValue() + 1)) + " "
    ...
    public uint16 read_u16(uint32 address, boolean log_output) throws MemoryException
    {
        ...
        {
            log.error("Read u16 from unaligned addr: " + address.getValueAsHexString());
    ...
    public void write_u32(uint32 address, uint32 value) throws MemoryException
    {
        if((address.getValue()&0x3 ) != 0)
        {
            log.error("Write u32 to unaligned addr: " + address.getValueAsHexString());
            throw new MemoryException("Write u32 to unaligned addr: " + address.getValueAsHexString());
        }

        log.debug("Write u32 to addr: " + address.getValueAsHexString() + " value: " + value.getValueAsHexString());
        ...
        log.debug("Written: " + value.getValueAsHexString() + " -> " + memory.readByteAsString(address.getValue() + 3) + " " + memory.readByteAsString(address.getValue() + 2) + " "
    ...
    {
        log.debug("Write u8 to addr: " + address.getValueAsHexString() + " value: " + value.getValueAsHexString());

        memory.writeByte(address.getValue(), value.getValue());

        log.debug("Written: " + value.getValueAsHexString() + " -> " + memory.readByteAsString(address.getValue()));
        dumpMemory(new uint32(address.getValue() - 4), new uint32(address.getValue() + 4));
    }
