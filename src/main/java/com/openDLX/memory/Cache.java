...
import com.openDLX.PipelineConstants;
 import com.openDLX.datatypes.CacheReplacementPolicy;
 import com.openDLX.datatypes.CacheType;
 import com.openDLX.datatypes.DCacheWritePolicy;
 import com.openDLX.datatypes.RequestType;
 ...
 import com.openDLX.util.CacheAddressCalculator;
 import com.openDLX.util.CalculationHelper;
...
public abstract class Cache implements MemoryInterface {
	...
	private void initialize() throws CacheException, PipelineDataTypeException
	{
		...
		{
			Logger logger = context.getLogger("ICache");
		}
		else if(cache_type == CacheType.DCACHE)
		{
			Logger logger = context.getLogger("DCache");
		}
		else
		{
			Logger logger = context.getLogger("UnknownCache");
		}
		...
		log.info("Initialized " + cache_type + " with " + associativity + " ways, " + line_no + " lines, " + lines_per_set + " lines per set, " + words_per_line + " words per line, " + tag_size + " bits for tag, " + index_size + " bits for index, and " + block_offset_size + " bits for block offset. Cache is of type: " + this.getClass());
	}
	...
	public uint32 read_u32(uint32 addr, boolean log_output) throws MemoryException 
	{
		...
		if(log_output)
		{
			log.debug("Read u32 from addr: " + addr.getValueAsHexString());
		}
		...
		if(isHit(addr))
		{
			...
			if(log_output)
			{
				log.debug("Hit in way " + way + " in cache line " + index + " for address " + addr.getValueAsHexString() + " value: " + value.getValueAsHexString());
				dumpCacheLine(index);
			}
		...
		else
		{
			...
			log.debug("Accessing way: " + way + " index: " + index);
			
			cache_memory[way][index].setLine(getTagFromAddress(addr), line);
			
			if(log_output)
			{
				log.debug("Miss in cache for address " + addr.getValueAsHexString() + " replaced cache line " + index + " in way " + way + " loaded value: " + value.getValueAsHexString());
				dumpCacheLine(index);
			}
	...
	public uint8 read_u8(uint32 addr, boolean log_output) throws MemoryException 
	{
		...
		if(log_output)
		{
			log.debug("Read u8 from addr: " + addr.getValueAsHexString());
		}
		...
		if(isHit(addr))
		{
			...
			if(log_output)
			{
				log.debug("Hit in way " + way + " in cache line " + index + " for address " + addr.getValueAsHexString() + " value: " + value.getValueAsHexString() + " (read byte " + (getBlockOffset(addr)&0x3) + " from word " + cache_memory[way][index].getWord(getBlockOffset(addr)&(~0x3)) + ")");
				dumpCacheLine(index);
			}
		...
		else
		{
			...
			if(log_output)
			{
				log.debug("Miss in cache for address " + addr.getValueAsHexString() + " replaced cache line " + index + " in way " + way + " loaded value: " + value.getValueAsHexString() + " (read byte " + (getBlockOffset(addr)&0x3) + " from word " + cache_memory[way][index].getWord(getBlockOffset(addr)&(~0x3)) + ")");
				dumpCacheLine(index);
			}
	...
	public void write_u32(uint32 addr, uint32 value) throws MemoryException 
	{
		...
		{
			log.error("Write u32 to unaligned addr: " + addr.getValueAsHexString());
			throw new CacheException("Write u32 to unaligned addr: " + addr.getValueAsHexString());
		}
		
		log.debug("Write u32 to addr: " + addr.getValueAsHexString() + " value: " + value.getValueAsHexString());
		...
		if(isHit(addr))
		{
			...
			log.debug("Hit in way " + way + " in cache line " + index + " for address " + addr.getValueAsHexString() + " old value: " + old_value.getValueAsHexString() + " new value: " + cache_memory[way][index].getWord(getBlockOffset(addr)));
			dumpCacheLine(index);
		...
		else
		{
			...
			log.debug("Miss in cache for address " + addr.getValueAsHexString() + " replaced cache line " + index + " in way " + way + " old_value: " + old_value.getValueAsHexString() + " new value: " + cache_memory[way][index].getWord(getBlockOffset(addr)));
			dumpCacheLine(index);
	...
	public void write_u8(uint32 addr, uint8 value) throws MemoryException 
	{
		...
		log.debug("Write u8 to addr: " + addr.getValueAsHexString() + " value: " + value.getValueAsHexString());
		...
		if(isHit(addr))
		{
			...
			log.debug("Hit in way " + way + " in cache line " + index + " for address " + addr.getValueAsHexString() + " old value: " + old_value.getValueAsHexString() + " new value: " + cache_memory[way][index].getByte(getBlockOffset(addr)) + " (written byte " + (getBlockOffset(addr)&0x3) + " of word " + cache_memory[way][index].getWord(getBlockOffset(addr)&(~0x3)) + ")");
			dumpCacheLine(index);
		...
		else
		{
			...
			log.debug("Miss in cache for address " + addr.getValueAsHexString() + " replaced cache line " + index + " in way " + way + " old_value: " + old_value.getValueAsHexString() + " new value: " + cache_memory[way][index].getByte(getBlockOffset(addr)) + " (written byte " + (getBlockOffset(addr)&0x3) + " of word " + cache_memory[way][index].getWord(getBlockOffset(addr)&(~0x3)) + ")");
			dumpCacheLine(index);
	...
	protected int getIndex(uint32 addr)
	{
		...
		log.debug("idx: " + Integer.toHexString(CalculationHelper.generateBitStringOfOnes(index_size)) + " index mask: 0x"  + Integer.toHexString(mask) + " value: " + addr.getValueAsHexString() + " result: 0x" + Integer.toHexString((addr.getValue()>> block_offset_size) & mask));
...
//					break;
	...
	public boolean isHit(uint32 addr) throws CacheException
	{
		...
		log.debug("Accessing index: " + index);
	...
	{
		log.debug("Dumping cache content for index: 0x" + Integer.toHexString(index));
		for(int i = 0; i < associativity; i++)
		{
			log.debug("Way: " + i + " " + cache_memory[i][index].dumpLine());
		}
	}
